...
app.post('/faculty/approve', async (req, res) => {
  ...
  try {
    ...
    if (courseAllottedCheck.rows.length > 0) {
      console.log('Allotted record:', courseAllottedCheck.rows[0]);
    }
...
app.post('/student/:id/pay', async (req, res) => {
  ...
  try {
    ...
    const payment_url = `http://localhost:4000?student_id=${id}&semester=${semester}&amount=${amount_paid}`;
...
app.post('/faculty/mark-attendance', async (req, res) => {
  const { course_offering_id, date, attendance } = req.body;

  try {
    // Extract present student IDs from the attendance array
    const present_student_ids = attendance
...
app.post('/admin/declare-results', async (req, res) => {
  ...
  try {
    ...
    // Update System_Config which will trigger the database function to update CGPA
    const result = await pool.query(
    ...
    console.log('Results declared. System config updated:', result.rows[0]);
    res.json({
...
app.get('/student/:id/results', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      console.log('Student results:', result.rows[0]);
      res.json(result.rows[0]);
    } else {
...
app.get('/student/:id/grades', async (req, res) => {
  ...
  try {
    ...
    console.log('Student grades:', result.rows);
    res.json(result.rows);
